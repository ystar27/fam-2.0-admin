...
export default function SideNav({ user }: any) {
  ...
  useEffect(() => {
    if (
      user?._id === "61af7811b864682357071f67" &&
      navLinks[0].name !== "Administrator"
    ) {
      navLinks.unshift({
        name: "Administrator",
        link: "/admin",
    ...
    <div className={"h-full overflow-y-auto"}>
      <div className={"flex flex-col items-center py-8 px-5"}>
        ...
        <button
          onClick={logout}
          className={
            "flex items-start w-full side-name-link mb-2 duration-150 text-left py-3 px-4 text-gray-500 cursor-pointer rounded-md"
          }
        >
