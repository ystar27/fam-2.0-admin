...
export default function Create({ setCreate, categories, rOpportunity, setROpportunity }: any) {
	...
	const createOpportunity = async (e) => {
		...
		try {
			if (data.title && data.endingDate) {
				...
				const createR = await axios.post("/opportunity", data);
				setROpportunity([...rOpportunity, createR.data.data]);
				setCreate(false);
			} else {
				setError(true);
			}
			...
			<form onSubmit={createOpportunity}>
				<div ref={slideRef} className={`max-w-lg slide-in min-w-min w-full duration-500 right-0 md:right-10 absolute top-40 bg-white flex flex-col`}>
					<div className={"p-5 flex items-center justify-between bg-gray-50"}>
						...
						<button className={"p-2 bg-white"} onClick={() => setCreate(false)}>
