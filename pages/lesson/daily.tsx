...
const Daily = ({ }: any) => {
	...
	}, [dailyLessons.module, dailyLessons.subModule, data.module, data.subModule]);
	...
	const onSelectChange = (e: any) => {
		if (e.target.value !== "" && e.target.name === "module") {
			...
			let selectedSubMdl: any = subModule.filter((submodule: { _id: any }) => submodule._id == e.target.value);
			setDailyLesson({
				...dailyLessons,
	...
	const updateDayLessons = (e, day) => {
		if (dailyLessons.lessons.filter((lesson) => lesson.day == day).length > 0) {
			...
			setDailyLesson({
				...dailyLessons,
				lessons: [...dailyLessons.lessons, { day, lesson: e.target.value || " " }],
			});
		}
	};
				...
				<div className="mx-auto container px-5 duration-300">
					<div className={"my-16"}>
						...
						<div style={{ color: "#B569D4" }} className={"flex items-center"}>
							<h5 className={"mr-2"}>Dashboard</h5> &gt; <h5 className={"ml-2"}>Daily Lesson</h5>
						</div>
					</div>
					<div>
						<div
						...
						>
							<div className={"grid grid-cols-3 gap-5 md:gap-10 justify-between"}>
									...
									<div className={"mb-4 w-full"}>
										...
										>
											...
											{issue.map((e: any, idx: number) => (
												<option key={idx} value={e._id} className={"hover:bg-purple-400"}>
													{e.title}
												</option>
											))}
										</select>
						...
						<form onSubmit={submitLessons}>
							<div>
								{inputedLessons.length > 0 && (
									...
									>
										<h3 className={"mb-5 text-lg text-gray-600 font-semibold font-mono"}>Available Daily Lessons</h3>
										<div className={"grid grid-cols-3 gap-5 md:gap-8"}>
											{inputedLessons.map((e: any, i: number) => (
												<InputedLessonItem key={i} item={e} index={i} lessons={data.lessons} updateDayLessons={updateDayLessons} />
											))}
										</div>
										<div className={"flex justify-end mt-8"}>
											<button className={"px-10 py-2 rounded-md text-white font-semibold"} style={{ backgroundColor: "#b569d4" }}>
												Save
											</button>
										</div>
									</div>
						...
						{data.subModule && data.subModule?.duration && (
							<form onSubmit={submitLessons}>
								<div>
									...
									>
										<h3 className={"mb-5 text-lg text-gray-600 font-semibold font-mono"}>Unset Daily Lessons</h3>
										<div className={"grid grid-cols-3 gap-5 md:gap-8"}>
											{Array.from(Array(data.subModule?.duration).keys()).map((e, i) => (
												<DayItem key={i} index={i} lessons={data.lessons} updateDayLessons={updateDayLessons} />
											))}
										</div>
										<div className={"flex justify-end mt-8"}>
											<button type={"submit"} className={"px-10 py-2 rounded-md text-white font-semibold"} style={{ backgroundColor: "#b569d4" }}>
												Save
											</button>
										</div>
									</div>
