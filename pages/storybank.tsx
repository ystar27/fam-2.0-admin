...
const Dashboard = dynamic(() => import("../components/Layouts/Dashboard/Dashboard"));
...
function StoryBank({ subMod }: any) {
	...
	const GetStoriesFunc = async () => {
		let subModules = [];

		if (router.query?.subModule) {
			setRelatedSubModule(true);
			let res = await axios.get(`/story/${router.query?.subModule}/submodule`);
	...
	const updateStory = async () => {
		try {
			...
			setSubModules(subModules.map((submodule) => (submodule._id == activeStory._id ? res.data.data : submodule)));
	...
	const deleteStoryFunc = () => {
		axios
			...
			.then((res) => {
				const uSubMdl = subModules.filter((submodule) => submodule._id !== activeStory._id);
	...
	return (
		<div className={"flex flex-col h-screen"}>
			<Head />
			<Navbar />
			{deleteStory && <Delete deleteFunc={deleteStoryFunc} setIDelete={setDeleteStory} message={"Approve delete"} />}
			<Dashboard>
				<div className="mx-auto container px-5 duration-300">
					<div className={"my-16"}>
						...
						<div style={{ color: "#B569D4" }} className={"flex ems-center"}>
							<h5 className={"mr-2"}>Dashboard</h5> &gt; <h5 className={"ml-2"}>Story Bank</h5>
						</div>
					</div>
					<div>
							...
							<div className={"w-full bg-white rounded py-6 mb-32"} style={{ boxShadow: "0px 4px 45px rgba(0, 0, 0, 0.04)" }}>
								<div className={"px-5 flex justify-end"}>
									<Link href={"/storybank/new"} passHref>
										<a
											className={"py-1 px-2 w-56 text-center text-white rounded"}
											style={{
												background: "#B569D4",
								...
								{relatedSubModule ? (
									<ModuleStoryTable rows={subModules} setActiveStory={setActiveStory} setEditStory={setEditStory} setDeleteStory={setDeleteStory} />
								) : (
									<StoryTable rows={subModules} setActiveStory={setActiveStory} setEditStory={setEditStory} setDeleteStory={setDeleteStory} />
								)}
							</div>
						)}
					</div>
