...
function Idea({ project }: any) {
	...
	const loadProject = async () => {
		...
		insertProject(project.data.data);
	};

	const insertProject = (project: any) => {
		if (project.length > 0) {
	...
	return (
		<div className={"flex flex-col h-screen"}>
			<Head />
			<Navbar />
			{iDelete && <Delete deleteFunc={deleteIProject} setIDelete={setIDelete} message="Delete project idea" />}
			{details && <ProjectDetails activeProject={activeProject} setDetails={setDetails} />}
			<Dashboard>
				<div className="mx-auto container px-5 duration-300">
					<div className={"my-16"}>
						...
						<div style={{ color: "#B569D4" }} className={"flex items-center"}>
							<h5 className={"mr-2"}>Dashboard</h5> &gt; <h5 className={"ml-2"}> Project Ideas</h5>
						</div>
					</div>
					<div>
